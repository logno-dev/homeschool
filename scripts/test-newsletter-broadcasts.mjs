...
try {
  ...
  const sanitizedEmail = emailContent.sanitizeEmailHtml('<table style="width:100%;border-collapse:collapse;position:fixed"><tr><td style="color:#e60000;padding:8px" onclick="bad()">{{firstName}}</td></tr></table><script>bad()</script><a href="javascript:bad()">Bad link</a>')
  assert.match(sanitizedEmail, /<table/)
  assert.match(sanitizedEmail, /width:100%/)
  assert.match(sanitizedEmail, /border-collapse:collapse/)
  assert.match(sanitizedEmail, /color:#e60000/)
  assert.match(sanitizedEmail, /padding:8px/)
  assert.match(sanitizedEmail, /\{\{firstName\}\}/)
  assert.ok(!/script|onclick|javascript|position/.test(sanitizedEmail))
