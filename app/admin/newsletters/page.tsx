  ...
  return <AdminLayout userName={userName} activeTab="newsletters">
    <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[18rem_1fr]">
        <aside className="rounded-lg bg-white p-4 shadow"><h2 className="font-semibold text-gray-900">Drafts & sends</h2><div className="mt-3 space-y-2">{messages.map((entry) => <button key={entry.id} onClick={() => selectMessage(entry.id)} className={`w-full rounded-md border px-3 py-3 text-left ${selectedId === entry.id ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:bg-gray-50'}`}><p className="truncate text-sm font-medium text-gray-900">{entry.subject || 'Untitled message'}</p><p className="mt-1 text-xs capitalize text-gray-500">{entry.kind === 'bulk_email' ? 'Bulk email' : 'Newsletter'} · {entry.status.replaceAll('_', ' ')}</p></button>)}{!messages.length && <p className="py-4 text-sm text-gray-500">No messages yet.</p>}</div></aside>
