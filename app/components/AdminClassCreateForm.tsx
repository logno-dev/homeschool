...
const initialForm = {
  ...
  teacherId: '', teacherName: 'Staff Instructor', coTeacherId: '', coTeacher: '', studentTeacherChildId: '', studentCoTeacherChildId: '', classroomNeeds: '', registrationFeeExempt: false, requiresFee: false, feeAmount: '', schedulingRequirements: ''
}
...
export default function AdminClassCreateForm({ sessions, teachers, children, onCreated }: Props) {
  ...
  const selectedCoTeacher = teachers.find((teacher) => teacher.id === form.coTeacherId)
  const availableStudentCoTeachers = selectedCoTeacher ? children.filter((child) => child.familyId === selectedCoTeacher.familyId) : []
      ...
      {open && <form onSubmit={submit} className="mt-5 space-y-5 border-t pt-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
         ...
         <label className="block text-sm font-medium text-gray-700">Student Teacher<select value={form.studentTeacherChildId} disabled={!selectedTeacher} onChange={(event) => setForm({ ...form, studentTeacherChildId: event.target.value })} className="mt-1 block w-full rounded-md border border-gray-300 bg-white px-3 py-2 font-normal disabled:bg-gray-100"><option value="">No student teacher</option>{availableStudentTeachers.map((child) => <option key={child.id} value={child.id}>{child.firstName} {child.lastName} (Grade {child.grade})</option>)}</select><span className="mt-1 block text-xs font-normal text-gray-500">Only children in the selected teacher's family are available.</span></label>
          <div className="grid gap-4 sm:grid-cols-2"><label className="text-sm font-medium text-gray-700">Co-Teacher Parent<select value={form.coTeacherId} onChange={(event) => { const teacher = teachers.find((option) => option.id === event.target.value); setForm({ ...form, coTeacherId: event.target.value, coTeacher: teacher ? `${teacher.firstName} ${teacher.lastName}` : '', studentCoTeacherChildId: '' }) }} className="mt-1 block w-full rounded-md border border-gray-300 bg-white px-3 py-2 font-normal"><option value="">Write in below</option>{teachers.filter((teacher) => teacher.id !== form.teacherId).map((teacher) => <option key={teacher.id} value={teacher.id}>{teacher.firstName} {teacher.lastName} ({teacher.email})</option>)}</select><input value={form.coTeacher} disabled={Boolean(form.coTeacherId)} onChange={(event) => setForm({ ...form, coTeacher: event.target.value })} placeholder="Write-in co-teacher name" className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 font-normal disabled:bg-gray-100" /></label><label className="text-sm font-medium text-gray-700">Student Co-Teacher<select value={form.studentCoTeacherChildId} disabled={!selectedCoTeacher} onChange={(event) => setForm({ ...form, studentCoTeacherChildId: event.target.value })} className="mt-1 block w-full rounded-md border border-gray-300 bg-white px-3 py-2 font-normal disabled:bg-gray-100"><option value="">No student co-teacher</option>{availableStudentCoTeachers.map((child) => <option key={child.id} value={child.id}>{child.firstName} {child.lastName} (Grade {child.grade})</option>)}</select><span className="mt-1 block text-xs font-normal text-gray-500">Select the parent first, then choose a child from that family.</span></label></div>
          <label className="block text-sm font-medium text-gray-700">Classroom Needs<input value={form.classroomNeeds} onChange={(event) => setForm({ ...form, classroomNeeds: event.target.value })} placeholder="TV/projector, supplies" className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 font-normal" /></label>
