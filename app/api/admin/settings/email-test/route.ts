...
const fakeValues: Record<string, string> = { firstName: 'Alex', familyName: 'Rivera Family', requesterName: 'Alex Rivera', applicantName: 'Alex Rivera', name: 'Alex Rivera', email: 'alex@example.com', sessionName: 'Fall 2026 Session', classNames: 'Art Explorers, Science Lab', className: 'Art Explorers', description: 'A sample class description.', gradeRange: 'K-2', reason: 'Sample reason', scholarshipType: 'Partial', requestedAmount: '$60.00', additionalInfo: 'Additional sample details.', resetUrl: 'https://example.com/reset-password', totalAmount: '$250.00', amountPaid: '$250.00', balanceDue: '$0.00', donationAmount: '$50.00', dueDate: 'October 1, 2026', billingStatement: sampleStatement, invoice: sampleInvoice }
...
export async function POST(request: Request) {
  try {
    ...
    await sendTestNotificationEmail({ type, to: body.to, subject: body.subject, template: sanitizeEmailHtml(body.template), variables })
