...
export default function AdminSessionDetailPage() {
  ...
  const [form, setForm] = useState({ name: '', startDate: '', endDate: '', classTeachingRegistrationStartDate: '', classTeachingRegistrationEndDate: '', description: '', isActive: false })
  ...
  const load = async () => {
    ...
    setWindows(((await windowResponse.json()).windows || []).map((entry: WindowEntry) => ({ groupId: entry.groupId, startDate: formatWindowForInput(entry.startDate, false), endDate: formatWindowForInput(entry.endDate, true) })))
    setForm({ name: nextSession.name, startDate: nextSession.startDate, endDate: nextSession.endDate, classTeachingRegistrationStartDate: nextSession.classTeachingRegistrationStartDate || '', classTeachingRegistrationEndDate: nextSession.classTeachingRegistrationEndDate || '', description: nextSession.description || '', isActive: nextSession.isActive })
  }
  ...
  const save = async (event: React.FormEvent) => {
    ...
    } catch (error) { setMessage(error instanceof Error ? error.message : 'Unable to save session') } finally { setBusy(false) }
  }
  ...
  return <AdminLayout userName={userName} activeTab="sessions"><main className="max-w-4xl mx-auto py-6 px-4 sm:px-6 lg:px-8"><div className="flex items-center justify-between gap-4"><div><Link href="/admin/sessions" className="text-sm text-blue-600 hover:text-blue-800">← Sessions</Link><h1 className="mt-2 text-2xl font-bold text-gray-900">{session.name}</h1><p className="text-sm text-gray-600">{displayDate(session.startDate)} - {displayDate(session.endDate)}</p></div><button onClick={deleteSession} disabled={busy} className="rounded-md border border-red-200 bg-red-50 px-4 py-2 text-sm font-semibold text-red-700 disabled:opacity-50">Delete Session</button></div>
    <div className="mt-6 space-y-6"><section className="rounded-lg bg-white p-6 shadow"><h2 className="text-lg font-semibold text-gray-900">Session Details</h2><form onSubmit={save} className="mt-5 space-y-5"><div className="space-y-4"><label className="block text-sm font-medium text-gray-700">Session Name<input required value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 font-normal" /></label><label className="block text-sm font-medium text-gray-700">Description<input value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 font-normal" /></label><label className="block text-sm font-medium text-gray-700">Start Date<input required type="date" value={form.startDate} onChange={(event) => setForm({ ...form, startDate: event.target.value })} className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 font-normal" /></label><label className="block text-sm font-medium text-gray-700">End Date<input required type="date" value={form.endDate} onChange={(event) => setForm({ ...form, endDate: event.target.value })} className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 font-normal" /></label></div><label className="flex items-center gap-2 text-sm text-gray-700"><input type="checkbox" checked={form.isActive} onChange={(event) => setForm({ ...form, isActive: event.target.checked })} className="h-4 w-4 rounded border-gray-300 text-blue-600" />Set as active session</label>
      <div className="rounded-md border border-amber-100 bg-amber-50 p-4"><h3 className="text-sm font-semibold text-amber-900">Parent Class-Teaching Request Window</h3><p className="mt-1 text-sm text-amber-800">Controls when parents may submit class-teaching requests. This is separate from student registration windows.</p><div className="mt-3 grid gap-4 sm:grid-cols-2"><label className="text-sm font-medium text-gray-700">Opens<input required type="date" value={form.classTeachingRegistrationStartDate} onChange={(event) => setForm({ ...form, classTeachingRegistrationStartDate: event.target.value })} className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 font-normal" /></label><label className="text-sm font-medium text-gray-700">Closes<input required type="date" value={form.classTeachingRegistrationEndDate} onChange={(event) => setForm({ ...form, classTeachingRegistrationEndDate: event.target.value })} className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 font-normal" /></label></div></div>
       <div className="rounded-md border border-blue-100 bg-blue-50 p-4"><h3 className="text-sm font-semibold text-blue-900">Registration Windows by User Group</h3><p className="mt-1 text-sm text-blue-800">Users can register when any assigned group window is open. Times use the configured site timezone.</p><div className="mt-4 space-y-3">{groups.map((group) => { const entry = windows.find((window) => window.groupId === group.id) || { groupId: group.id, startDate: '', endDate: '' }; return <div key={group.id} className="grid gap-3 sm:grid-cols-3 sm:items-end"><p className="text-sm font-medium text-gray-900">{group.name}</p><label className="text-xs font-medium text-gray-700">Opens (site timezone)<input type="datetime-local" value={entry.startDate} onChange={(event) => updateWindow(group.id, 'startDate', event.target.value)} className="mt-1 block w-full rounded-md border border-gray-300 px-2 py-2 text-sm" /></label><label className="text-xs font-medium text-gray-700">Closes (site timezone)<input type="datetime-local" value={entry.endDate} onChange={(event) => updateWindow(group.id, 'endDate', event.target.value)} className="mt-1 block w-full rounded-md border border-gray-300 px-2 py-2 text-sm" /></label></div> })}</div></div><button disabled={busy} className="rounded-md bg-blue-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{busy ? 'Saving...' : 'Save Session'}</button></form>{message && <p className="mt-4 rounded-md bg-blue-50 p-3 text-sm text-blue-800">{message}</p>}</section><div><SessionFeeConfig sessionId={session.id} sessionName={session.name} inline /></div></div></main></AdminLayout>
}
